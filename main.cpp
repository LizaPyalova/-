...
public:
    IntegerSet(const std::vector<int> &get) {
        my_set = get;
...
int main() {
    IntegerSet one({1, 4, 8, 8});
    IntegerSet two({1, 4, 7});
    My_set final = one.add(two);
    for (auto elem: final.my_set) {
        std::cout << elem << " ";
    }
    StringSet first({"a", "b", "a", "c"});
    StringSet second({"a", "c", "x"});
    My_set deez_set = first.add(second);
    for (auto e: deez_set.my_string_set) {
        std::cout << e << " ";
    }
